...
func TestBuildStartupBash(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("BuildStartup() returned error: %v", err)
	}
	...
	if !reflect.DeepEqual(startup, want) {
		t.Fatalf("startup = %#v, want %#v", startup, want)
	}
}
...
func TestBuildStartupZsh(t *testing.T) {
	...
	if !reflect.DeepEqual(startup.Args, []string{"-d"}) {
		t.Fatalf("args = %#v, want %#v", startup.Args, []string{"-d"})
	}

	zshrc := filepath.Join(wantZDOTDIR, ".zshrc")

	data, err := os.ReadFile(zshrc)
	if err != nil {
		t.Fatalf("ReadFile(%q) returned error: %v", zshrc, err)
	}
