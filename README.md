...
8. Authenticated bundled runtime assets.
...
authentication
        |
        v
verify protected embedded bundle
        |
        v
...
The authenticated runtime payload should ultimately be embedded inside the
...
No runtime payload should be written to disk before authentication succeeds.

Set `UNISHELL_STARTUP_TIMING=1` to print verification, stale cleanup,
runtime setup, decompression/extraction, and multiplexer reconciliation
timings to stderr.
...
New version 4 bundles use HMAC-SHA256 as a token gate. The compressed runtime payload is stored in plaintext inside the bundle and binary. The tag covers only a fixed challenge, so this format provides neither confidentiality nor payload integrity.
...
| token-gate HMAC-SHA256 tag  |
| plaintext compressed payload|
+-----------------------------+
```

The tag is an HMAC over the format prefix and a fixed label; verification does not read or hash the payload and does not derive a key with Argon2id. This intentionally accepts modified payloads. The tag also permits offline token guesses, and a user able to modify the binary can bypass an application-level gate. This is only a deterrent against casual use.

Version 3 integrity-authenticated bundles and version 2 encrypted bundles remain readable for compatibility. Release builds generate version 4 token-gate bundles. Rebuild the binary to receive the new format and launch-time behavior.
...
The production authenticated bundle must be generated by the release/build
...
- versioned bundle encoding and verification;
    
- HMAC-SHA256 token gate for version 4 bundles;
    
- version 3 integrity verification and version 2 AES-GCM decryption for compatibility.
...
application code
+
authenticated runtime bundle
+
required metadata
```

The version 4 runtime payload is token-gated but not encrypted inside the binary.
...
wrong credential
wrong token
payload modification is accepted
corrupt token-gate header
truncated bundle
invalid archive
path traversal
permission denied
...
```text
correct token -> gate opens
wrong token -> authentication failure
payload modification -> accepted with a valid token
invalid/truncated gate header -> invalid bundle error
```
...
Never include authenticated payload contents in errors.
...
HMAC-SHA256 token gate for version 4 bundles
+
Argon2id/AES-256-GCM compatibility for version 2 bundles
+
...
When executed:
    ...
    5. Check the token gate and extract the required tools/configuration.
...
verify the authenticated bundle.
...
Version 4 uses HMAC-SHA256 over a short fixed challenge to gate the token. It does not provide confidentiality or payload integrity, and permits offline token guessing.

Version 3 integrity-authenticated and version 2 encrypted bundles remain supported for compatibility. New bundle builds use version 4; rebuild release binaries and embedded bundles to use the token-only gate.
...
Do not expose extracted runtime payloads unnecessarily.
