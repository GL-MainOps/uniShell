...
canonical binary name
      |
      v
future ELF / musl / executable validation
      |
      v
runtime bundle
```
...
These fields correspond to later acquisition validation stages.

When adding a tool, set the requirements according to the intended runtime  
contract of that artifact.

Do not claim that an artifact is static or musl-linked merely because its  
filename contains `musl`.

The actual artifact must eventually pass the corresponding validation.
...
## 16. Adding a New Tool: Contributor Procedure
...
## 18. What Adding a Tool Does Not Do
...
## 19. Current Roadmap Relationship
